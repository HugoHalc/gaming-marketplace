...
export function SiteFooter() {
  return (
    <footer className="border-t border-white/[0.06] bg-black/15 py-10 sm:py-12">
      <Container>
        <div className="flex flex-col gap-8 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <Logo />
            <p className="mt-3 max-w-sm text-sm leading-6 text-[var(--muted-foreground)]">
              Premium gaming services with transparent configuration, secure checkout architecture, and clear order progress.
            </p>
          </div>
          <nav className="flex flex-wrap gap-x-6 gap-y-3" aria-label="Footer navigation">
            {footerLinks.map((item) => (
              <Link key={item.href} href={item.href} className="text-sm text-[var(--muted-foreground)] transition-colors hover:text-white">
                {item.label}
              </Link>
            ))}
          </nav>
        ...
        <div className="mt-9 flex flex-col gap-3 border-t border-white/[0.06] pt-6 text-xs text-[var(--muted-foreground)] sm:flex-row sm:items-center sm:justify-between">
          <p>© 2026 VantaBoost. All rights reserved.</p>
