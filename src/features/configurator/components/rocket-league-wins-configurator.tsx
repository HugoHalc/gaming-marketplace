...
function firstRankForFamily(familyKey: string) {
  return familyKey === "supersonic-legend"
    ? "supersonic-legend"
    : `${familyKey}-1`;
}
        ...
        {rankFamilies.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`mt-2 line-clamp-2 min-h-8 w-full text-center text-[11px] font-semibold leading-4 transition-colors duration-200 ${selected ? "text-white" : "text-white/68 group-hover/rank:text-white/90"}`}>{item.label}</span>
            </button>
          );
        })}
      </div>
...
export function RocketLeagueWinsConfigurator({ gameSlug, service }: Props) {
  ...
  return (
    <div className="grid gap-4 pb-[calc(5.25rem+env(safe-area-inset-bottom))] xl:grid-cols-[minmax(0,1fr)_23rem] xl:items-start xl:pb-0">
              ...
              <div className="flex items-end justify-between gap-4">
                ...
                <div className="rounded-[1.15rem] border border-white/[0.07] bg-white/[0.018] p-4 sm:p-5">
                  <input
                    ...
                    aria-describedby={!quantityIsValid ? "wins-quantity-error" : undefined}
                    type="number"
                    inputMode="numeric"
                    min={1}
                    max={20}
      ...
      <aside id="wins-summary" className="scroll-mt-28 xl:scroll-mt-24 xl:sticky xl:top-24">
        <div className="space-y-3">
          <div className="overflow-hidden rounded-[1.6rem] border border-white/[0.09] bg-[#070A08] shadow-[0_26px_70px_-46px_rgba(0,0,0,.95)]">
            <RocketLeagueOrderSummaryHeader serviceTitle="Competitive Wins" isLoading={isLoading} ready={minimumOrderSatisfied} />
            ...
            <Button
              ...
              aria-describedby={minimumOrderBlocked ? "wins-minimum-order" : undefined}
              onClick={createOrder}
            >
              {isCreatingOrder ? (
                <>
